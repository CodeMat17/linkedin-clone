    ...
    return (
        <div className="space-y-2 min-w-max max-w-lg">
            ...
            <div
                ...
                overflow-hidden relative flex flex-col items-center
                text-center border border-gray-300 dark:border-none">
                <div className="relative w-full h-14 bg-gray-400"></div>
                <Avatar
                    onClick={signOut}
                    src={session?.user?.image}
                    className="!h-14 !w-14 !border-2 !absolute !top-6 !cursor-pointer"
                />
                <div className="mt-5 py-4 space-x-0.5">
                    <p
                        className="hover:underline decoration-purple-700
                        underline-offset-2 cursor-pointer">
                        {session?.user?.name}
                    </p>
                    <p className="px-4 text-black/60 dark:text-white/75">
                        {session?.user?.email}
                    </p> 
                </div>
                
                <div
                    className="hidden md:inline text-left dark:text-white/75 text-sm">
                    <div className="font-medium sidebarButton space-y-0.5">
                        <div className="flex justify-between space-x-2">
                            <h4>Who viewed your profile</h4>
                            <span className="text-blue-500">632</span>
                        </div>
                        <div className="flex justify-between space-x-2">
                            <h4>Views of your post</h4>
                            <span className="text-blue-500">2,890</span>
                        </div>
                    </div>
           ...
           {/* Bottom */}
            <div
                className="hidden px-4 md:flex bg-white dark:bg-gray-700 
                text-black/70 dark:text-white/75 rounded-lg
                overflow-hidden flex-col space-y-2 pt-2.5 sticky top-20
